...
export async function getPortfolioActivity({
  ...
  page = 1,
  itemsPerPage = 10,
}: GetPortfolioActivityParams): Promise<PortfolioActivityApiResponse> {
  const queryParams = [`items_per_page=${itemsPerPage}`, `page=${page}`];

  try {
    return await client.fetch(
      `/portfolio/${walletAddress}/activity?${queryParams.join("&")}`,
    );
  } catch (error) {
    throw new Error(
      "failed to fetch portfolio activity : " + (error as string),
    );
  }
}
