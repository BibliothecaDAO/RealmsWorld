...
export async function getCollectionActivity({
  ...
  page = 1,
  itemsPerPage = 10,
}: GetCollectionActivityParams): Promise<CollectionActivityApiResponse> {
  try {
    const queryParams = [`items_per_page=${itemsPerPage}`, `page=${page}`];

    return await client.fetch(
      `/collections/${collectionAddress}/activity?${queryParams.join("&")}`,
    );
  } catch (error) {
    throw new Error(
      "failed to fetch collection activity : " + (error as string),
    );
  }
}
